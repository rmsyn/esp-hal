...
use esp_hal::sdio::{Config, Mode, Pins, Sdio, command::Cmd0, response::R1};
...
fn main() -> ! {
    ...
    loop {
        ...
        esp_println::println!("command: {:?}", &cmd_buf[..Cmd0::LEN]);

        // let exp_packet = Cmd52::new();
        //
        // assert_eq!(Cmd52::try_from_bytes(&cmd_buf), Ok(exp_packet));

        let exp_res = R1::new().into_bytes();

        ctx.sdio
            .write_response_raw(&exp_res)
